...
function esd_ZxdpublicwzaGz() {
	...
	esdJumpNodeTagLwLb(esd_mainnav_Array[7].esd_lastfocus, function () {
		...
		esd_mainnav_Array[7].esd_frame.hide();
		esd_removeDisplay();
		$(".crumbs a:first").focus();
	});

	esdJumpNodeTagLwLb($(".crumbs a:first"), function () {
		esd_mainnav_Array[7].esd_frame.show();
		esd_mainnav_Array[7].esd_lastfocus.focus();
	}, function () {
		if($(".crumbs a:last")[0]==$(".crumbs a:eq(0)")[0]){
			$(".crumbs").next().find("a:first").focus();
		}else{
			$(".crumbs a:eq(0)").siblings("a").focus();
		}
	});
...
function esd_wzaGz(){/* 改造代码开始 */
	$(function(){
		...
		$(".reg-list li").each(function(){
			esd_setinputTitle($(this).find("input"),$(this).find("input").attr("placeholder"));
		})
		esd_setinputTitle($("#userName"),"请输入您的用户名，注意：用户名设置成功后不可修改");
		esd_setinputTitle($("#passWord"),"请输入您的登录密码，注意：登录密码为6-20位字母、数字或符号");
		esd_setinputTitle($("#confirmPassWord"),"再次输入您的登录密码");
		esd_setinputTitle($("#cardType"),"请选择您的证件类型");
		esd_setinputTitle($("#idTypeCode_second"),"您使用的是中国居民身份证");
		esd_setinputTitle($("#idTypeCode_wgjzz"),"您使用的是外国人永久居留身份证");
		esd_setinputTitle($("#idTypeCode_gatjjz"),"港澳台居民居住证");
		esd_setinputTitle($("#passengerType"),"请选择旅客类型");
		esd_setinputTitle($("#id_limit_date_end"),"请输入证件有效期截止日期");
		esd_setinputTitle($("#born_date"),"请输入出生日期");
		esd_setinputTitle($("#nation"),"请选择国家/地区");
		esd_setinputTitle($(".mr25").parent(".r-txt").find("input:eq(0)"),"请选择您的性别，当前为男，可以使用键盘的上下键进行选择，按空格键选中");
		esd_setinputTitle($(".mr25").parent(".r-txt").find("input:eq(1)"),"请选择您的性别，当前为女，可以使用键盘的上下键进行选择，按空格键选中");
		esd_setinputTitle($("#checkAgree"),"点击表示您已阅读并同意遵守《中国铁路客户服务中心网站服务条款》 《隐私权政策》");
	})
}
	}, 
 }
